...
export default function AktualnosciTab({ news }: Props) {
  const router = useRouter();
  const [formOpen, setFormOpen] = useState(false);
  const [editItem, setEditItem] = useState<NewsItem | undefined>();
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <p className="text-[14px] text-sand-600">
          {news.length === 0 ? "Brak aktualnosci." : `${news.length} wpisow`}
        </p>
        <button
          onClick={openCreate}
          ...
          {news.map((item) => (
            ...
            >
              {item.images[0] ? (
                ...
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-sand-100">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-sand-400">
