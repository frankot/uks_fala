...
export default function HeroStrip({
  ...
  const tagColors = {
    pool: { line: "bg-pool-400", text: "text-pool-400", subtitle: "text-pool-300" },
    coral: { line: "bg-coral-400", text: "text-coral-500", subtitle: "text-coral-300" },
  };
    ...
    <div className="relative overflow-hidden bg-deep-900 pt-28 pb-16">
      ...
      <div className="relative z-10 mx-auto max-w-[1240px] px-5 sm:px-8">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <span className={`text-[12px] font-bold uppercase tracking-[0.2em] ${c.text}`}>
            {tag}
          </span>
        </div>
        <h1 className="font-editorial mt-3 text-[clamp(2rem,5vw,3.5rem)] font-bold leading-[1.06] tracking-[-0.02em] text-white">
          {title}
          {subtitle && <span className={`block ${c.subtitle}`}>{subtitle}</span>}
        </h1>
        {description && (
